...
public class CarBehaviour : MonoBehaviour
{
    ...
    private HashSet<int> processing = new HashSet<int>();
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (Package != null)
        {
            var id = Package.GetInstanceID();
            if (processing.Contains(id)) {
                Debug.LogWarning($"skipped dublicated id={id}");
                return;
            }
            processing.Add(id);
            StartCoroutine(Example(Package, other));
        }
    }
    IEnumerator Example(PackageBehaviour packageBehaviour, Collider other)
    {
        var scale = new Vector3(-0.0005f, -0.0005f, -0.0005f);
        for (var i = 0; i < 180; ++i)
        {
            yield return new WaitForSeconds(1/25.0f);
            packageBehaviour.gameObject.transform.Rotate(Vector3.up * i);
            packageBehaviour.gameObject.transform.localScale += scale;
        }
